...
- 预加载的 chunk 会在父 chunk 加载时，以并行方式开始加载。预获取的 chunk 会在父 chunk 加载结束后开始加载。
- 预加载的 chunk 具有中等优先级，并立即下载。预获取的 chunk 在浏览器闲置时下载。
- 预加载的 chunk 会在父 chunk 中立即请求，用于当下时刻。预获取的 chunk 会用于未来的某个时刻。
- 浏览器支持程度不同。
